...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SKW address (e.g. %1)").arg("idaeAtzaCGnJ5SUPXKoFxtgzbvv5nfSKWDn"));
#endif
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert SKW:// to SKW:
    //
    //    Cannot handle this later, because SKW:// will cause Qt to see the part after // as host,
...
{
    return GetAutostartDir() / "SKW.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a SKW.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the SKW app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add SKW app to startup item list
